variable "resource_group_name" {
  type        = string
  description = "The name of the resource group in which to create the Static Web App."
}

variable "location" {
  type        = string
  description = "The Azure region where the Static Web App should be created."
}

variable "static_web_app_name" {
  type        = string
  description = "The name of the Static Web App."
}

variable "sku_name" {
  type        = string
  description = "The SKU name of the Static Web App."
  default     = "Free"
}

variable "branch" {
  type        = string
  description = "The branch of the repository to deploy."
}

variable "repository_url" {
  type        = string
  description = "The URL of the repository to deploy."
}

variable "location_source" {
  type        = string
  description = "The location of the source code within the repository."
}

variable "app_location" {
  type        = string
  description = "The location of the application code."
}

variable "api_location" {
  type        = string
  description = "The location of the API code."
}

variable "app_artifact_location" {
  type        = string
  description = "The location of the built application artifacts."
}

variable "identity_type" {
  type        = string
  description = "the type of managed identity to assign to this static web app."
  default     = "SystemAssigned"
}
